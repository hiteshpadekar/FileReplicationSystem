            ...
            if(req.type == REQ_RECOVERDIR) {
                ...
                do {
				  ...
				  //decrypt((char *)&rsp,sizeof(rsp));
                        ...
                        /* send file name */
				  //encrypt_buff((char *)&dent.d_name);
                        ret = write(client_fd, dent.d_name, len);
				  //decrypt_buff((char *)&dent.d_name);
				  ...
				  //decrypt((char *)&rsp,sizeof(rsp));
                        ...
                        do {
                            ...
                            len += ret;
                            if(ret > 0) {
						 //encrypt_buff((char *)&buf);
                                ret = write(client_fd, buf, ret);
						//decrypt_buff((char *)&buf);
                            }
                        } while(len < fsize);
                        printf("Sent File......\n"); 
                        fclose(fp);
         ...
         if(req.type == REQ_DIRINFO) {
            ...
            do {
               ...
               if(strstr(dent.d_name, FILE_FILTER) != NULL) {
				  ...
				  //encrypt_buff((char *)&stat_buf.st_mtime);
                  ret = write(client_fd, &stat_buf.st_mtime, sizeof(time_t));
				  //decrypt_buff((char *)&stat_buf.st_mtime);
               }
            } while(next_dent);
            closedir(dirp);
